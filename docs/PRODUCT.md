...
- Per-category document sections, developer-approved, displayed as tabs on the client view (specs/014-category-sections)
...
- **Per-category document sections** (specs/014-category-sections, supersedes the AI-categorization approach of specs/013): the category list is a **fixed, product-wide set of four** — `overview` (The project), `how_it_works` (How it works), `planning` (Roadmap) and `other` (Other information) — identical in every project, not invented by the AI and not configurable per project. Adding a category is a code change; the list is deliberately small because adding one later is cheap while removing one that already carries content is not.

  The unit of content is a **section**, not a document. A developer's document is usually messy — some purpose, a diagram, a couple of dates, interleaved — so the AI does not label the document; it splits what the document *says* across the categories it genuinely addresses, producing one plain-language section per (document, category) pair, in both languages, in a single analysis pass. Everything of substance must land in exactly one section, and anything fitting no specific category goes to `other`, so nothing is silently dropped. A document that addresses three categories yields three sections with **different** content — which is what makes the client's tabs differ, the defect that made 013's tabs all show the same documents.

  A contributor reviews what the AI filed where, section by section: approve, reject, or re-file into another category (while still proposed). Only approved sections of a published resource reach the client, and publishing is refused when nothing is approved. On the client view, each category with content becomes a tab holding an accordion — first section expanded, content readable with no navigation, the source document one click away.
...
- [x] **AI resource categorization — category lifecycle and data model**: resolved by specs/014-category-sections (2026-08-09). Freeform per-project categories were tried in specs/013 and failed — reusing a project's existing categories turned into a feedback loop that converged on a handful of generic labels, so every client tab listed the same documents. The answer is a fixed product-wide list of four categories, with the AI extracting per-category content rather than labelling whole documents. Review is a separate step from `publish()`: a contributor approves, rejects or re-files each section independently, and publishing is then refused unless at least one section is approved. Re-filing is allowed only while a section is still proposed — moving one after approval would silently pull it out of a tab a client is already reading.
