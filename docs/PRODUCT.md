...
- A two-layer documentation pipeline: a developer-validated reference layer built from the project's documents, and the plain-language version derived from it that a client reads under category tabs (specs/015-document-reference-layer)
...
- **File/attachment management** — specs/011-project-resources shipped uploads (PDF/Word/PNG/JPEG) and Notion-page resources. Its per-document publication flow (`processing → ready_for_review → published`, gated by an explicit `publish()` action) was removed by specs/015: a document is an **input** now, not something a client reads, so there is nothing per-document to publish. A document's own lifecycle is just `pending → absorbed | failed`.
- **A two-layer documentation pipeline** (specs/015-document-reference-layer, supersedes the per-document sections of specs/014 and the AI-categorization of specs/013). The category list is a **fixed, product-wide set of four** — `overview` (The project), `how_it_works` (How it works), `planning` (Roadmap) and `other` (Other information) — identical in every project, not invented by the AI and not configurable per project. Adding a category is a code change; the list is deliberately small because adding one later is cheap while removing one that already carries content is not.
  ...
  **Two layers, and the separation is the point.** The *reference* layer is the developer's own working document: organised and professional, in ordinary language, never vulgarized. It is clean in FORM (no repetition, no padding) but exhaustive in SUBSTANCE — where those pull against each other, exhaustiveness wins, because the raw sources are never read again afterwards and anything dropped there is lost for good. The *client* layer is derived from the reference and is the only thing a client ever sees. It is always regenerated from the reference, never from a previous client version — which is what stops detail eroding across successive rewrites.

  **One validation gate, at the reference layer**, where the facts, dates, figures and contradictions actually live. A contributor reviews a queue of independent per-category drafts: accept, or refuse — discarding the draft (the previously validated version stays live) or sending it back with an instruction in their own words, capped at three attempts. Approving is the only act that makes anything client-visible; the vulgarized version follows from it without a second review queue. Adding a document and deleting one are the same operation in both directions: each category's stored per-document extracts are re-merged over what remains.

  **The system may ask before it guesses.** Up to five questions per category, ranked by impact, and only where the answer would change what the client is told — never about style or completeness for its own sake. They are skippable by construction: the corresponding point is marked as open inside the reference text at the moment the question is asked, so accepting a draft with questions outstanding leaves it visibly unresolved rather than silently arbitrated.
...
- [x] **AI resource categorization — category lifecycle and data model**: resolved in two passes. specs/014-category-sections (2026-08-09) settled the taxonomy: freeform per-project categories were tried in specs/013 and failed — reusing a project's existing categories turned into a feedback loop that converged on a handful of generic labels, so every client tab listed the same documents. The answer is a **fixed product-wide list of four**, with the AI extracting per-category content rather than labelling whole documents. That part stands.
  ...
  What 014 got wrong was the **unit**: one section per (document, category). It broke in real use on the second document, which was appended as a separate block rather than merged, leaving the client to reconcile several passages about the same subject. specs/015-document-reference-layer (2026-08-11) makes the **category** the unit and splits the pipeline in two — a validated reference layer, and the client layer derived from it. Per-document publication and the move-a-section action are gone with it: with one body per category there is nothing to move, and a document is an input rather than something a client reads.
