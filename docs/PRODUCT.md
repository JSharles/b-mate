...
- A canonical, traceable documentary source built from all project documents, followed by developer-validated factual references and an immutable plain-language client release (`specs/016-canonical-document-workflow`, superseding specs/015 for the active implementation).
...
- **AI-assisted simplification (vulgarization)** — task vulgarization (`apps/api/src/task-vulgarization/`, specs/007-current-task-vulgarization) and canonical documentary processing (`apps/api/src/documentation/`, specs/016-canonical-document-workflow) use durable, provider-neutral generation operations. Anthropic is the current configured primary; provider/model/fallback policy remains modifiable without changing the documentary domain.
- **File/attachment management** — uploads (PDF/Word/PNG/JPEG) and immutable Notion snapshots are source contributions. A document is never published directly: it is extracted into attributable observations and incorporated into the project source. Its lifecycle distinguishes reception, extraction, consolidation, incorporation, failure, removal-in-progress and a retained removal tombstone.
- **Canonical documentary workflow** (`specs/016-canonical-document-workflow`, superseding specs/015 as the active implementation). The category list remains a **fixed, product-wide set of four** — `overview` (The project), `how_it_works` (How it works), `planning` (Roadmap) and `other` (Other information) — identical in every project, not invented by the AI and not configurable per project.

  **The unit of content is the category, at project scale — not the document.** 014 made it the document and it broke on the second one: a document completing a subject already covered was appended as a separate block, leaving the client to reconcile several passages about the same thing. A category now holds one continuous body, and a document that says something about three categories feeds three of them.

  **Three deliberate layers.** The immutable canonical source keeps stable information identities and provenance across documents, clarifications, corrections, language changes and removals. The *factual reference* is the developer's review surface, organised and exhaustive without client-facing simplification. The *client release* is derived only from an accepted factual reference under a versioned editorial profile. It is never regenerated from an older client rewrite, preventing progressive loss of detail.

  **One human validation gate, at the factual reference layer**, where facts, dates, figures and contradictions live. A contributor reviews independent per-category drafts: accept, discard while retaining the previous client release, or request a factual correction in their own words. Editorial requests such as “shorter” or “more pedagogical” are routed to the project editorial profile instead of being misinterpreted as factual corrections. Approving is the only act that can prepare new client-visible content.

  **The system asks before it guesses.** Material contradictions and ambiguities produce ranked, attributable clarifications with no hidden count cap. A contributor can answer or deliberately leave a point open. Open points keep a stable identity through the factual reference and client release, so uncertainty is published explicitly rather than silently arbitrated.

  **Publication is atomic.** Each client category is immutable and a project points to one complete release. A new accepted category or confirmed editorial-profile change is prepared separately; the client keeps seeing the previous release until the replacement is complete. Provider outages, exhausted credits, invalid output or manual retry therefore delay publication without overwriting validated content.
...
- [x] **AI resource categorization — category lifecycle and data model**: resolved in two passes. specs/014-category-sections (2026-08-09) settled the taxonomy: freeform per-project categories were tried in specs/013 and failed — reusing a project's existing categories turned into a feedback loop that converged on a handful of generic labels, so every client tab listed the same documents. The answer is a **fixed product-wide list of four**, with the AI extracting per-category content rather than labelling whole documents. That part stands.
  ...
  What 014 got wrong was the **unit**: one section per (document, category). It broke in real use on the second document, which was appended as a separate block rather than merged. specs/015 made the category the unit; specs/016-canonical-document-workflow (2026-08-11) completes that decision with an immutable canonical source, stable provenance, contributor clarifications/corrections, factual per-category review, versioned editorial profiles, provider-neutral durable generation and atomic client releases. Per-document publication is gone: a document is an input, not something a client reads.
